...
class Controllercatalogtaskreport  extends Controller {
   ...
   public function archive() {
	...
	$insert_data = $this->db->query("INSERT IGNORE INTO `oc_task_" . $year . "` SELECT * FROM `oc_task` WHERE `date` <= '" . $last_month_end . "' AND `status` = 'done';");
	$delete_last_data = $this->db->query("DELETE FROM `oc_task` WHERE `date` <= '" . $last_month_end . "' AND `status` = 'done';");
